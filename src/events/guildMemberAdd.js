...
module.exports = {
    ...
    run: async (client, member) => {
        ...
        let db = await client.database({ guildId: guild.id}, "modules_welcome");
        ...
        try {
            ...
            if(channel && typeof channel.send === 'function') {
                ...
                client.log(`[WELCOME] No se encontro el canal especificado en ${guild.id}`)
                await sendLog({ message: "**Error!** No se especifico un canal donde enviar las bienvenidas.", guild })
            }

        } catch(e) {
            client.log("[WELCOME]", e);
            await sendLog({ message: "**Error!** Configuracion incorrecta del modulo de Bienvenidas", guild })
        }
